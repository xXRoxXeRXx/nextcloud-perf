...
func NewClient(url, user, pass string, logFunc func(string)) *Client {
	...
	return &Client{
		...
		Password: pass,
		Client: &http.Client{
			Timeout: 60 * time.Second, // Increased timeout for large chunks/moves
		},
		LogFunc: logFunc,
	}
}
...
func (c *Client) GetCapabilities() (*CapabilitiesResponse, error) {
	...
	req.SetBasicAuth(c.Username, c.Password)
	req.Header.Set("OCS-APIRequest", "true")
	req.Header.Set("User-Agent", "Mozilla/5.0 (Windows) mirall/3.15.3 (build 20250107) (Nextcloud Performance Tool)") // Mimic official client
...
func (c *Client) UploadChunked(remotePath string, data io.Reader, totalSize int64) (time.Duration, error) {
	...
	if err != nil { return 0, err } // If network fails completely
